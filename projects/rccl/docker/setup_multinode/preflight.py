...
def run_preflight(cfg):
    ...
    if action == Action.RUN_ONLY:
        _check_image_exists(cfg)
    ...
    if needs_build or action == Action.RUN_ONLY:
        log("  Image     : {}".format(cfg.image_tag))
        log("  Dockerfile: {}".format(cfg.dockerfile))
    if action in (Action.RUN, Action.RUN_ONLY, Action.LAUNCH_ALL):
...
def _check_gpus(cfg):
    ...
    if count > 0:
        ...
        _warn("No GPUs detected (set --gpus manually if needed)")
...
def _check_image_exists(cfg):
    # type: (Config) -> None
    if cfg.runtime and cfg.runtime.image_exists():
        _ok("Image '{}' exists".format(cfg.image_tag))
    else:
        _fail(
            "Image '{}' not found (--run-only requires a pre-built image)".format(
                cfg.image_tag
            )
        )
...
def _check_ssh_keys(cfg):
    ...
    if cfg.ssh.key:
        ...
        _ok("SSH keys: will auto-generate (--ssh-keygen)")
    else:
        if cfg.action in (Action.LAUNCH_ALL, Action.VERIFY):
            _warn("No SSH keys configured (use --ssh-key or --ssh-keygen)")
        else:
